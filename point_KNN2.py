...
def KNN(data, labels, point, k):
    ...
    label = findLabels([idxs])[0]
    i = [x[1] for x in neighbors]
    ns = np.array([data[j] for j in i])
    return label, ns

p = [30, 50]
l, ns = KNN(data, labels, p, 5)
...
plt.scatter(ns[:, 0], ns[:, 1], facecolor = 'none', edgecolors='lime', s=200, linewidths=3)
plt.legend()
plt.show()
